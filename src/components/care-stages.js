...
const CareStages = ({ data }) => {
  const fallbackStageTitles = [
    ...
    "Safe labour & delivery management",
    "Postpartum care",
    "Menstrual & hormonal disorder treatment",
  ...
  useLayoutEffect(() => {
    ...
    const getGap = () => {
      ...
      return parseFloat(styles.columnGap || styles.gap || "50") || 50
    }
    ...
    const getOuterContentWidth = () => {
      const styles = window.getComputedStyle(outerRef.current)
      const horizontalPadding =
        (parseFloat(styles.paddingLeft) || 0) +
        (parseFloat(styles.paddingRight) || 0)

      return Math.max(0, outerRef.current.clientWidth - horizontalPadding)
    }

    const sizeCards = () => {
      const cardsVisible = getCardsVisible()
      const gap = getGap()
      const outerW = getOuterContentWidth()
    ...
    const getMaxOffset = () =>
      Math.max(0, trackRef.current.scrollWidth - getOuterContentWidth())

    const getMaxStep = () => {
      const stepWidth = getStepWidth()
      return stepWidth > 0 ? Math.ceil(getMaxOffset() / stepWidth) : 0
    }
    ...
    const initScrollTrigger = () => {
      ...
      tweenRef.current = gsap.to(trackRef.current, {
        ...
        scrollTrigger: {
          ...
          onLeave: () => {
            gsap.set(trackRef.current, { x: -getMaxOffset() })
        ...
        <div className="care-stages__track-outer" ref={outerRef}>
          <div className="care-stages__track" id="careTrack" ref={trackRef}>
            {careAcrossHorizontalSlider.map((item, index) => {
              ...
              const suppliedTitle = cleanTitle(item?.sectionTitle)
              const cardTitle =
                suppliedTitle ||
                fallbackStageTitles[index] ||
                cleanTitle(item?.sectionImage?.node?.altText) ||
                "Women's healthcare"
              const imageAlt = item?.sectionImage?.node?.altText || cardTitle
